...
def prepare_data_for_clustering(input_data):
    ...
    def agglomerative_clustering(gdf, df, n_clusters, name):
        ...
        # Fiji straddles the antimeridian. Shift western-hemisphere pieces
        # eastward for this diagnostic map only so the archipelago is not
        # drawn as a line spanning the entire world.
        plot_gdf = gdf.copy()
        plot_gdf["geometry"] = plot_gdf.geometry.apply(
            lambda geom: translate(geom, xoff=360)
            if geom is not None and not geom.is_empty and geom.centroid.x < 0
            else geom
        )
        ax = plot_gdf.plot(column='clusters_yield', cmap='viridis', legend=True)
        if not plot_gdf.empty and plot_gdf.total_bounds[2] > 180:
            ax.set_xlabel(
                "Longitude (degrees east; values >180 cross the dateline)"
            )
